 ...
 *      ─ Import and call startDiscountCleanupJob() in your app entry point.
 *
...
const CRON_EXPRESSION    = process.env.CLEANUP_CRON           || "0 2 * * *"; // 2 AM daily
...
export async function runCleanup() {
  ...
  try {
    ...
    // bulkExpireStale uses a single updateMany against the (validUntil, status) index.
    ...
    console.log(`[DiscountCleanup] Hard-deleted ${deleted} expired discount(s) older than ${HARD_DELETE_DAYS} days`);

    const elapsed = Date.now() - start;
    console.log(`[DiscountCleanup] Done in ${elapsed}ms — expired: ${expired}, deleted: ${deleted}`);
...
/**
 * Call this once during app startup to register the scheduled job.
 *
 * @example
 * // app.js / server.js
