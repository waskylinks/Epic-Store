...
export const getReturnOverview = handleAsyncError(async (req, res, next) => {
    ...
    Order.aggregate([
      ...
      },
      {
        $group: {
          ...
          totalReturns: { $sum: 1 },
          requested: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'requested'] }, 1, 0] }
          },
          approved: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'approved'] }, 1, 0] }
          },
          inTransit: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'in_transit'] }, 1, 0] }
          },
          received: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'received'] }, 1, 0] }
          },
          completed: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'completed'] }, 1, 0] }
          },
          rejected: {
            $sum: { $cond: [{ $eq: ['$returnInfo.status', 'rejected'] }, 1, 0] }
          },
  ...
  // Get total delivered orders for return rate calculation
  ...
  // Calculate trend
  const trend = previous.totalReturns > 0
    ? Math.round(((current.totalReturns - previous.totalReturns) / previous.totalReturns) * 100 * 100) / 100
    : 0;
  ...
  // Average processing time (requested to completed)
...
export const getReturnsByProduct = handleAsyncError(async (req, res, next) => {
  ...
  // Get all returns with product details
  ...
  // Get total sales for each product to calculate return rate
  ...
  const productsWithReturnRate = await Promise.all(
    returnsByProduct.map(async (item) => {
      ...
      const product = await Product.findById(item._id).select('name images category pricing');
  ...
  // Filter out null products and sort
  ...
  const response = {
    ...
    summary: {
      ...
      avgReturnRate: sortedProducts.length > 0
        ? Math.round(sortedProducts.reduce((sum, p) => sum + p.returnRate, 0) / sortedProducts.length * 100) / 100
        : 0
    }
  };
...
export const getReturnsByCategory = handleAsyncError(async (req, res, next) => {
  ...
  // This requires joining with Product collection
  const returnsByCategory = await Order.aggregate([
    {
      $match: {
        'returnInfo.status': { $nin: ['none', 'rejected'] }
      }
  ...
  // Combine data
  const salesMap = new Map(
    salesByCategory.map(item => [item._id, item.totalSales])
  );
  ...
  const response = {
    categories: categoriesWithReturnRate
  };
...
export const getRefundOverview = handleAsyncError(async (req, res, next) => {
    ...
    Order.aggregate([
      ...
      },
      {
        $group: {
          ...
          totalRefunds: { $sum: 1 },
          requested: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'requested'] }, 1, 0] }
          },
          approved: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'approved'] }, 1, 0] }
          },
          processing: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'processing'] }, 1, 0] }
          },
          completed: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'completed'] }, 1, 0] }
          },
          rejected: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'rejected'] }, 1, 0] }
          },
          failed: {
            $sum: { $cond: [{ $eq: ['$refundInfo.status', 'failed'] }, 1, 0] }
          },
  ...
  const previous = previousRefunds[0] || {
    totalRefunds: 0,
    totalRefundAmount: 0
  };

  // Calculate trends
  const refundTrend = previous.totalRefunds > 0
    ? Math.round(((current.totalRefunds - previous.totalRefunds) / previous.totalRefunds) * 100 * 100) / 100
    : 0;

  const amountTrend = previous.totalRefundAmount > 0
    ? Math.round(((current.totalRefundAmount - previous.totalRefundAmount) / previous.totalRefundAmount) * 100 * 100) / 100
    : 0;

  // Get refund reasons
  const refundReasons = await Order.aggregate([
    {
      $match: {
        'refundInfo.requestedAt': { $gte: currentPeriodStart },
        'refundInfo.status': { $ne: 'none' }
...
export const getRefundsByPaymentMethod = handleAsyncError(async (req, res, next) => {
  ...
  const refundsByMethod = await Order.aggregate([
    {
      $match: {
        'refundInfo.status': 'completed'
      }
  ...
  // Get total orders by payment method for refund rate
  ...
  const ordersMap = new Map(
    ordersByMethod.map(item => [item._id, item.totalOrders])
  );
