...
// FIX OPS1: Removed unused HandleError import — was imported but never called.
...
export const getSLABreachAnalytics = handleAsyncError(async (req, res, next) => {
  ...
  const stats = breachStats[0] || { totalOrders: 0, breachedOrders: 0, avgDelayDays: 0 };
  const breachRate =
    stats.totalOrders > 0
      ? Math.round((stats.breachedOrders / stats.totalOrders) * 100 * 100) / 100
      : 0;

  const response = { summary: { ...stats, breachRate }, breaches };
...
export const getShippingCarrierPerformance = handleAsyncError(async (req, res, next) => {
  ...
  const response = { carriers: enrichedCarrierPerformance };
...
export const getCancellationAnalytics = handleAsyncError(async (req, res, next) => {
  ...
  const response = {
    summary: { ...stats, cancellationRate },
