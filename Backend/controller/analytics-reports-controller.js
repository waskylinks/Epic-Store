...
export const generateBusinessPerformanceReport = handleAsyncError(async (req, res, next) => {
  ...
  const report = {
    ...
    period: { start: periodStart, end: periodEnd, label: timeframe },
...
export const generateSalesReport = handleAsyncError(async (req, res, next) => {
  const { timeframe = "month", startDate, endDate, groupBy = "day" } = req.query;
  ...
  const report = {
    ...
    period: { start: periodStart, end: periodEnd, groupBy },
    generatedAt: new Date(),
    summary: {
...
export const generateCustomerReport = handleAsyncError(async (req, res, next) => {
  ...
  const [
    ...
    CustomerAnalytics.aggregate([
      { $group: { _id: "$risk.churnPrediction", count: { $sum: 1 } } }
    ]),
    CustomerAnalytics.aggregate([
      {
        $group: {
...
export const generateProductReport = handleAsyncError(async (req, res, next) => {
  ...
  const [topProductsByRevenue, categoryPerformance, inventoryStatus] = await Promise.all([
    Order.aggregate([
      {
        $match: {
          createdAt: { $gte: periodStart, $lte: periodEnd },
...
export const generateFinancialReport = handleAsyncError(async (req, res, next) => {
  ...
  const [financialData, refundData] = await Promise.all([
    Order.aggregate([
      {
        $match: {
          createdAt: { $gte: periodStart, $lte: periodEnd },
  ...
  const refunds = refundData[0] || { totalRefunded: 0, refundCount: 0 };
...
export const exportReportCSV = handleAsyncError(async (req, res, next) => {
  const { reportType, timeframe = "month", startDate, endDate } = req.query;
...
const getExecutiveSummary = async (startDate, endDate) => {
  ...
  return {
    ...
    avgOrderValue:
      orders > 0
        ? Math.round(((revenue[0]?.totalRevenue || 0) / orders) * 100) / 100
        : 0
  };
};
...
const getRevenueAnalysis = async (startDate, endDate) => {
  ...
  return analysis[0] || { totalRevenue: 0, shippingRevenue: 0, taxRevenue: 0, productRevenue: 0 };
};
...
const getSalesAnalysis = async (startDate, endDate) => {
  ...
  return { byStatus: ordersByStatus, byPaymentMethod: ordersByPaymentMethod };
};
...
const getOperationalMetrics = async (startDate, endDate) => {
  const [fulfillment, returns] = await Promise.all([
    Order.aggregate([
      ...
      { $group: { _id: null, avgFulfillmentDays: { $avg: "$fulfillmentTime" } } }
    ]),
...
const generateCustomersCSV = async () => {
  ...
  let csv =
    "Customer Name,Email,Total Revenue,Total Orders,AOV,RFM Segment,Value Tier\n";
