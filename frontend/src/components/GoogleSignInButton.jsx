...
function GoogleSignInButton({ text = 'Continue with Google' }) {
    ...
    return (
        ...
        >
             ...
             * in both idle and loading states.
             *
             * FIX: Google G sits on a white tile (#fff, border-radius 6px)
             * as required by Google Sign-In Branding Guidelines.
             * The coloured paths must never float on a tinted surface.
             */}
            <span className="oauth-icon" aria-hidden="true">
                {loading ? (
                    <span className="oauth-spinner" />
                ) : (
                    <span className="oauth-google-tile">
